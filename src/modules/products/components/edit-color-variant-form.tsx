...
import EditStockInputForm from './edit-stock-input-form';
...
export default function EditColorVariantForm({
  variantIndex,
  onRemove,
  form,
  onRemoveImage,
  ...
  const totalStock = (
    form.watch(`variants.${variantIndex}.sizes`) || []
  ).reduce((sum, s) => sum + (isNaN(+s.stock) ? 0 : +s.stock), 0);
  const id = useId();
  const { t } = useTranslation();
  const [defaultImages, setDefaultImages] = useState(
    form.formState.defaultValues?.variants?.[variantIndex]?.images as Array<{
      id: string;
      url: string;
    }>
  );
