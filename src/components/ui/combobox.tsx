...
>(({ className, ...props }, ref) => (
  <ComboboxPrimitive.Anchor
    ...
    ref={ref}
    className={cn(
      'relative flex h-9 w-full items-center justify-between gap-2 rounded-md border border-input bg-transparent p-2 shadow-xs data-focused:ring-1 data-focused:ring-ring dark:bg-input/30 dark:hover:bg-input/50',
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ComboboxPrimitive.Cancel
    ...
    ref={ref}
    className={cn(
      'absolute top-1/2 right-1 flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-sm bg-background opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none',
      className
    )}
    {...props}
  />
